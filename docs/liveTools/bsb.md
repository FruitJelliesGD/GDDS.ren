# Bilibili Better Share Button
###### <Badge type="tip" text="发行版" />
#### 替换B站原有分享按钮功能，点击分享后将当前页面信息和短链接格式化复制到剪切板  

https://greasyfork.org/zh-CN/scripts/503586-%E6%9B%B4%E5%A5%BD%E7%9A%84b%E7%AB%99%E5%88%86%E4%BA%AB%E6%8C%89%E9%92%AE  
...
(function() {
    ...
    function cleanTitle(ignoredTitle, isLive) {
        ...
        try {
            ...
            if (isLive || window.location.hostname === 'live.bilibili.com') {
                ...
                // ===== 标题 XPath 兜底列表 =====
                const titleXPaths = [
                    '//*[@id="app"]/div[3]/div[1]/div[1]/div/span',
                    '//*[@id="app"]/div[3]/div[1]/div[2]/div/span'
                ];

                // ===== 作者 XPath 兜底列表 =====
                const authorXPaths = [
                    '//*[@id="app"]/div[3]/div[1]/div[1]/div[2]/div[1]',
                    '//*[@id="app"]/div[3]/div[1]/div[2]/div[2]/div[1]',
                    '//*[@id="app"]/div[3]/div[1]/div[3]/div[2]/div[1]'
                ];
                ...
                try {
                    // ===== 作者（按顺序兜底）=====
                    authorText = '';
                    for (const xp of authorXPaths) {
                        authorText = getTextByXPath(xp);
                        if (authorText) break;
                    }

                    // ===== 标题（按顺序兜底）=====
                    titleText = '';
                    for (const xp of titleXPaths) {
                        titleText = getTextByXPath(xp);
                        if (titleText) break;
                    }

                    // ===== 标题最终兜底 =====
                    if (!titleText) {
                        titleText = authorText ? `${authorText}的动态` : '一条动态';
                    }
    ...
    function showSuccessMessage(message) {
        // 优先使用 BetterShare（视频页等）
        let anchorEl = document.getElementById('BetterShare');

        // opus 侧边栏兜底
        if (!anchorEl) {
            anchorEl = document.querySelector('.better-opus-side-share');
        }
        ...
        const fontSizeValue = parseFloat(computed.fontSize) || 14;
        ...
        Object.assign(messageDiv.style, {
            position: 'fixed',
            padding: `${fontSizeValue * 0.5}px ${fontSizeValue}px`,
            backgroundColor: '#000000',
            color: '#ffffff',
            borderRadius: `${fontSizeValue * 0.5}px`,
            boxShadow: `0 ${fontSizeValue * 0.1}px ${fontSizeValue * 0.5}px rgba(0,0,0,0.2)`,
            zIndex: 9999,
        ...
        if (anchorEl !== document.body) {
            ...
            messageDiv.style.left = `${rect.left + rect.width / 2 - msgRect.width / 2}px`;
            messageDiv.style.top = `${rect.top - msgRect.height - 10}px`;
        } else {
            // body 兜底：居中
    ...
    function injectOpusSideShareButton() {
        const observer = new MutationObserver(() => {
            const toolbar = document.querySelector('.side-toolbar__box');
            if (!toolbar) return;

            // 防止重复插入
            if (toolbar.querySelector('.better-opus-side-share')) return;

            const actions = toolbar.querySelectorAll('.side-toolbar__action');
            if (!actions.length) return;

            // 克隆一个现有 action，样式完全一致
            const shareAction = actions[actions.length - 1].cloneNode(true);
            shareAction.classList.add('better-opus-side-share');
            ...
            <svg width="24" height="24" viewBox="0 0 24 24">
                <path fill="currentColor"
                  d="M20.25 6a2.75 2.75 0 1 0-5.33.956L8.606 10.23a2.75 2.75 0 1 0-.01 3.55l6.208 3.679a2.75 2.75 0 1 0 .687-1.336l-6.27-3.716a2.8 2.8 0 0 0 .001-.806l6.6-3.423A2.75 2.75 0 0 0 20.25 6M17.5 4.75a1.25 1.25 0 1 1 0 2.5a1.25 1.25 0 0 1 0-2.5m-11 6a1.25 1.25 0 1 1 0 2.5a1.25 1.25 0 0 1 0-2.5m11 6a1.25 1.25 0 1 1 0 2.5a1.25 1.25 0 0 1 0-2.5"/>
            </svg>
            <span class="side-toolbar__action-text">分享</span>
        `;

            shareAction.addEventListener('click', async (e) => {
                e.stopPropagation();

                const opusRoot = document.querySelector('.bili-opus-view');
                if (!opusRoot) return;

                const did = opusRoot.dataset.did;
                if (!did) return;

                const longUrl = `https://www.bilibili.com/opus/${did}`;
            ...
            toolbar.appendChild(shareAction);
        });
